...
## 17. Early stopping
This activity assumes that you already know (tentatively) what hyperparameters (epochs, number of neurons, activations, etc.) are good for your dataset. For this activity, find a regression dataset of your choice and split into training and validation set. There are two objectives in this activity: a) implement automatic stopping of training if the accuracy does not improve for certain epochs, and b) implement automatic saving of the best model (best on the validation set).

* Lectures: [Early stopping](https://youtu.be/WsdpElbo9c4) 
